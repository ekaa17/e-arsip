...
class FileManagerDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        $dataTable = (new EloquentDataTable($query))
            ->addColumn('action', 'file-manager.action')
            ->addIndexColumn()
            ->editColumn('user_id', function (FileManagement $val) {
                return $val->user->name;
            })
            ->addColumn('action', function (FileManagement $val) {
                return view('pages.admin.setting.file-manager.action', ['file' => $val]);
            })
            ...
            ->editColumn('updated_at', function (FileManagement $val) {
                $diffHuman = Carbon::parse($val->updated_at)->diffForHumans();
                $formattedDate = Carbon::parse($val->updated_at)->format('d M Y, H:i');
                return <<<HTML
                    <div>
                        <p class="m-0">{$formattedDate}</p>
                        <small class="text-muted">{$diffHuman}</small>
                    </div>
                HTML;
            })
