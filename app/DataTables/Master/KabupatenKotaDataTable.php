...
class KabupatenKotaDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', 'kabupatenkota.action')
            ->addIndexColumn()
            ->addColumn('action', function (KabupatenKota $row) {
            ...
            ->editColumn('created_at', function (KabupatenKota $kabupatenkota) {
                return $kabupatenkota->created_at->format('d, M Y H:i');
            })
            ->editColumn('updated_at', function (KabupatenKota $kabupatenkota) {
                return $kabupatenkota->updated_at->format('d, M Y H:i');
            })
            ->rawColumns(['action'])
            ->setRowId('id');
    }
